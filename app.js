...
var theport = process.env.PORT || 8080; // 8080 for deployment
...
db.once('open', function callback () {
    ...
    var appDir = process.argv[2] || '../app/dist'; //'../menam_art_map/app' and for deployment '../app/dist'
  ...
  ArtistSchema.methods.findDistLocByTarget = function(cb) {
    var query;
    query = this.model('Artist').find({});
    query.where('target', this.target);
    query.where('group', this.group);
  ...
  ArtistSchema.methods.findSourceByTargetAll = function(cb) {
    var query;
    query = this.model('Artist').find({});
    query.where('target', this.target);
    // query.distinct('source');
  ...
  app.get('/distLocbysourceartist/:target', function(req, res) {
    var artist;
    res.header('Access-Control-Allow-Origin', '*');
    res.header('Access-Control-Allow-Headers', 'X-Requested-With');
    artist = new Artist({
